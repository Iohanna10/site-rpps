<h1 class="title" style="margin-bottom: 20px!important;">Mídias da galeria</h1>
...
use App\Models\Functions\Functions;
    ...
    echo '<table>';
        echo '<thead>';
            echo '<tr>';
                echo '<th style="width: 30%;">Mídia</th>';
                echo '<th style="width: 50%;">Informações</th>';
                ...
                foreach ($data['midias'] as $key => $media) {
                    ...
                    echo '<tr id="'. $media['nome'] .'">';
                        echo "<td class='handler_sortable'>";
                            ...
                            if($media['tipo'] !== 'url'){
                                // caminho do arquivo 
                                $path = base_url() . 'dynamic-page-content/'. $url .'/assets/uploads/'. $media['tipo'] .'/photo-gallery/all-images/' . (new DateTime($data['data']))->format('Y/n/') . (new Functions)->nameDir($data['nome']) .'/'. $media['nome'];
                        ...
                        echo "<td><textarea type='text' name='infos_img' id='" . $media['nome'] . "' placeholder='Adicionar informações para a imagem'>". $media['info'] ."</textarea></td>";

                        echo "<td><i class='fa-light fa-trash-can remove' title='excluir' onclick='confirmModal(`Deseja mesmo excluir essa mídia da galeria?`, `ajaxRemove_galleryEdit`, `". $media['nome'] . "`)'></i></td>";
                    echo '</tr>';
                } 
            } 
            else {
