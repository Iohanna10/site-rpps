    ...
    echo '<table>';
        echo '<thead>';
            echo '<tr>';
                ...
                echo '<th style="width: 12.5%;">Remover</th>';
            echo '</tr>';
        echo '</thead>';
            ...
            if($data['dbData'] != null){
                foreach ($data['dbData'] as $key => $theme) {
                    ...
                    echo '<tr id="'. $theme['id'] .'">';
                        ...
                        echo "<td><i class='fa-light fa-trash-can remove' title='excluir' onclick='confirmModal(`Deseja mesmo excluir esse tema?`, `ajaxRemove_theme`, ". $theme['id'] . ")'></i></td>";
                    echo '</tr>';
                } 
            } else {
                echo '<tr>';
                    ...
                    echo '<td colspan="3">';
                        echo '<nav aria-label="navigation">';
                            echo "<ul class='pagination'>";
                                ...
                                // primeira pg
                                echo "<li class='page-item'><a class='page-link' onclick='listUserThemes(1, getFilters())'>Primeira</a></li>";

                                // páginas anteriores
                                for($prevPg = ($data['currentPg'] - $maxLinks); $prevPg <= ($data['currentPg'] - 1); $prevPg++){
                                    if($prevPg > 0){
                                        echo "<li class='page-item'><a class='page-link' onclick='listUserThemes($prevPg, getFilters())'>$prevPg</a></li>";
                                    }
                                }
                                ...
                                for($nextPg = ($data['currentPg'] + 1); $nextPg <= ($data['currentPg'] + $maxLinks); $nextPg++){
                                    if($nextPg <= $data['qntPg']){
                                        echo "<li class='page-item'><a class='page-link' onclick='listUserThemes($nextPg, getFilters())'>$nextPg</a></li>";
                                    }
                                }

                                // última página
                                echo "<li class='page-item'><a class='page-link' onclick='listUserThemes(". $data['qntPg'] .", getFilters())'>Última</a></li>";
