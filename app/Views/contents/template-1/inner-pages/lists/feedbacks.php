<head>
    ...
    <link href="https://db.onlinewebfonts.com/c/2c8f6c3c149c5a07410e52cfb3b2db2b?family=mfn-icons" rel="stylesheet">
    <link rel="stylesheet" href="<?= base_url("assets/css/template-1/common/tabs.css") ?>">
    ...
    <link rel="stylesheet" href="<?= base_url("assets/css/template-1/inner-pages/common/containers.css") ?>"> <!-- CSS containers -->
</head>
<body>
            ...
            <div class="two-inputs">
                <div>
                    ...
                    <select name="rating-note" id="rating-note">
                        <option value="false" <?php if($data['feedbacks']['rating'] == 'false'){echo 'selected';} ?>>Todas</option>
                        <option value="null" <?php if($data['feedbacks']['rating'] == 'null'){echo 'selected';} ?>>0</option>
                        <option value="0" <?php if($data['feedbacks']['rating'] == '0'){echo 'selected';} ?>>1</option>
                        <option value="1" <?php if($data['feedbacks']['rating'] == '1'){echo 'selected';} ?>>2</option>
                        <option value="2" <?php if($data['feedbacks']['rating'] == '2'){echo 'selected';} ?>>3</option>
                        <option value="3" <?php if($data['feedbacks']['rating'] == '3'){echo 'selected';} ?>>4</option>
                        <option value="4" <?php if($data['feedbacks']['rating'] == '4'){echo 'selected';} ?>>5</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="feedback-only" id="feedback-only">
                        <option value="false" <?php if($data['feedbacks']['onlyComments'] == 'false'){echo 'selected';} ?>>Não</option>
                        <option value="true" <?php if($data['feedbacks']['onlyComments'] == 'true'){echo 'selected';} ?>>Sim</option>
                    </select>
                </div>
            </div>
    ...
    <div class="feedbacks_container row mt-5 justify-content-center">
        <?php
            if(count($data['feedbacks']['dbData']) > 0) {
                foreach ($data['feedbacks']['dbData'] as $key => $feedback) {
    ...
    <?php if($data['feedbacks']['qntPg'] > 1){ // menu de navegação
                    ...
                    echo '<td colspan="4" style="background-color: transparent;">';
                        echo '<nav aria-label="navigation">';
                            echo "<ul class='pagination'>";
                                ...
                                // páginas anteriores
                                for($prevPg = ($data['feedbacks']['currentPg'] - $maxLinks); $prevPg <= ($data['feedbacks']['currentPg'] - 1); $prevPg++){
                                    if($prevPg > 0){
                                ...
                                // página atual
                                echo "<li class='page-item'><a class='page-link current-pg'>". $data['feedbacks']['currentPg'] ."</a></li>";

                                // páginas posteriores
                                for($nextPg = ($data['feedbacks']['currentPg'] + 1); $nextPg <= ($data['feedbacks']['currentPg'] + $maxLinks); $nextPg++){
                                    if($nextPg <= $data['feedbacks']['qntPg']){
                                ...
                                // última página
                                echo "<li class='page-item'><a class='page-link' onclick='feedbacks(". $data['feedbacks']['qntPg'] .",  getFiltersFeedbacks()[0],  getFiltersFeedbacks()[1])'>Última</a></li>";
