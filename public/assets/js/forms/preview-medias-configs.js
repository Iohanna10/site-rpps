...
function previewImg(input) {
    var previewContainer = input.labels[0],
    preview = previewContainer.querySelector(".preview-img-container");
    
    preview.innerHTML = ""; // limpar preview
    ...
    if(input.files.length === 0)
        resetPreview(preview, previewContainer);

    // css icon/text div.conteiner-icon 
    photoIcon(previewContainer.querySelector(".container-icon"), input.files.length);
}

// pré visualização das imagens do carrossel

function activePreview_carousel() {
    ...
    if(inputSliderImg != null){
        ...
        moreFiles.addEventListener("change", function(){
            ...
            if(moreFiles.files.length > 0){
                ...
                moreFiles.files = (new DataTransfer()).files;
            }
        })
...
function activePreview_infosURL() {
...
function organizePreview(previewInput = document.getElementById('img')){
    labelContainer = previewInput.labels[0];
    labelContainer.querySelector(".preview-img-container").dataset.idMedia = '';
    labelContainer.querySelector(".preview-img-container ~ .container-icon").style.display = 'flex'

    var btnRemove = document.querySelector(`#img_remove[for=${previewInput.id}]`);

    if(btnRemove == null){
        btnRemove = document.getElementById('img_remove');
    }

    btnRemove.style.display = 'none';

    previewImg(previewInput)
}

async function resetPreview(preview, container){
    if(document.querySelector(".form-cad") != null) {
        var
        form = document.querySelector(".form-cad"),
        pathVariation = typeof form.dataset.pathVariation !== 'undefined' ? form.dataset.pathVariation : '', // variação do caminho do arquivo
        idMedia = preview.dataset.idMedia, // pegar nome da imagem
        urls = [`${getUriRoute(`dynamic-page-content/${getInstitute()}/assets/uploads/img/${pathVariation}${idMedia}`)}`, `${getUriRoute(`dynamic-page-content/${getInstitute()}/assets/uploads/img/favicon/${idMedia}`)}`]; // urls para verificar realmente é um caminho de imagem
        
        for(const url of urls) {            
            if(await checkUrl(url)){
                mediaHTML = `<img src="${url}" alt="${idMedia}" class="preview-img">`;
                preview.innerHTML = mediaHTML; // adicionar preview da imagem
    
                // photoIcon(container.querySelector(".container-icon"), 1); // arrumar visualização da icon
            }
        };
    }
}
...
// Nome do PDF selecionado 
function activePreview_PDF() {
    if(document.querySelector('#pdf') != null){
        document.querySelector('#pdf').addEventListener("change", function() {
            document.querySelector('.name-pdf').innerHTML = allPdfs(this.files);
        });
